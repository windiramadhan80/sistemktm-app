...
@section('content')
    <main class="container h-100 mx-auto mb-5">
        <h1 class="h4 mb-3 fw-normal">Registrasi</h1>
            ...
            <div class="row justify-content-center">
                <div class="col-lg-5">
                        ...
                        <div class="row">
                            <div class="col-lg">
                                ...
                                <select class="form-select @error('tanggal_lahir') is-invalid @enderror"
                                    ...
                                    @for ($h = 1; $h <= 31; $h++)
                                        <option value="{{ $h }}"
                                            {{ $h == old('tanggal_lahir') ? 'selected' : '' }}>{{ $h }}
                                        </option>
                                    @endfor
                                </select>
                                @error('tanggal_lahir')
                            ...
                            <div class="col-lg">
                                <select class="form-select @error('tahun_lahir') is-invalid @enderror" name="tahun_lahir"
                                    ...
                                    @for ($t = 2022; $t >= 1990; $t--)
                                        <option value="{{ $t }}"
                                            {{ $t == old('tahun_lahir') ? 'selected' : '' }}>{{ $t }}</option>
                                    @endfor
                                </select>
                                @error('tahun_lahir')
                                    <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="text-start">
                        <select class="form-select @error('program_studi') is-invalid @enderror" name="program_studi"
                            ...
                            @for ($p = 0; $p < 4; $p++)
                                @php
                                    $prodi = ['Teknologi Produksi Tanaman Perkebunan', 'Teknologi Pengolahan Hasil Kelapa Sawit', 'Budidaya Perkebunan Kelapa Sawit', 'Manajemen Logistik'];
                                @endphp
                ...
                <div class="col-lg-5">
                    <div class="text-start mb-2">
                        <h6>Photos</h6>
                        <img src="/img/users.jpg" class="img-preview img-thumbnail img-fluid mb-1" width="80px">
                    ...
                    <div class="form-floating text-start">
                        <input type="number" name="nim" class="form-control @error('nim') is-invalid @enderror"
                            id="nim" placeholder="Nama Lengkap" value="{{ old('nim') }}">
                        @error('nim')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <label for="nim">Nomor Induk Mahasiswa</label>
                    </div>
                    <div class="form-floating text-start mt-1">
        ...
        <p class="mt-2 mb-3"><a class="text-decoration-none" href="/login">Sudah punya akun? silahkan login</a></p>
